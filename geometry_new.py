...
###
def calculate_bond_length(file_name, begin_pattern_str = r'\S*_b', \
                          end_pattern_str = r'\S*_e'):
    bond_length = []
    bond_vector = calculate_bond_vector(file_name, begin_pattern_str, \
                                        end_pattern_str)
    bond_length.append(bond_vector[0])
    bond_length.append(np.sqrt(np.sum(np.array(bond_vector[1])**2)))
    return bond_length
###
def main():
    print('bond vector')
    print(str(calculate_bond_vector(r'./database/example1.xsd', r'\S*_b', r'\S*_e1')))
    print('bond length')
    print(calculate_bond_length(r'./database/example1.xsd'))
    print('bond angle from one file')
    print(calculate_bond_angle_from1file(r'./database/example1.xsd'))
    print('bond angle from two file')
